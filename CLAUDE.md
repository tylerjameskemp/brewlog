...
- `src/data/storage.js` — All localStorage logic (~40 exported functions). CRUD for brews, beans, recipes, equipment, pour templates, active brew. Recipe helpers: `RECIPE_FIELDS` constant, `recipeEntityToFormState()`, `formStateToRecipeFields()`. Also: `deduplicateBeans()`, `renameBrewBean()`, `migrateGrindSettings()`, `migrateBloomToSteps()`, `migrateToSchemaV2()`, `migrateExtractRecipes()`, `migrateDropRecipeSteps()`, `normalizeSteps()`, `formatTime()`/`parseTime()`/`parseFlexTime()`, `exportData()`/`importData()`/`mergeData()`, `getUIPref()`/`setUIPref()`, `getLastBrewOfBean()`, `getChangesForBean()`, `computeTimeStatus()`. `getBrews()` sorts by `brewedAt` descending with module-level cache (`_brewsCache`/`_brewsCacheRaw`); all write functions call `_invalidateBrewsCache()`.
- `src/components/BrewScreen.jsx` — **Largest file (~1,250 lines).** Guided brew flow with 4 inline sub-components: `BeanPicker`, `RecipeAssembly`, `ActiveBrew`, `RateThisBrew`, plus `BrewSuccess`. Phase state machine: `pick → recipe → brew → rate → success`. Key helpers: `buildBrewRecord` (useCallback, shared by `handleFinishBrew` and `handleLogWithoutTimer`), `buildRecipeFromEntity` (pre-fill from recipe entity or default template). Equipment section in RecipeAssembly allows per-brew equipment selection. All recipe fields always editable (no edit/done toggle). Skip-timer mode via "Log without timer" button bypasses ActiveBrew phase. Recipe save prompts handled by BrewSuccess (post-brew divergence detection). Communicates with App.jsx via narrow callback interface.
- `src/components/BrewForm.jsx` — Edit-only brew form. Used exclusively for editing existing brews from History (via `editBrew` prop). New brew creation goes through BrewScreen. Single StepEditor editing `steps` (actuals) with inline diff annotations against `recipeSnapshot.steps`. Post-save recipe update prompt when steps differ from plan. Preserves `recipeSnapshot`, `stepResults`, `method`, `grinder`, `dripper`, `filterType` on save.
- `src/components/BrewHistory.jsx` — Timeline of past brews with auto-diff badges. Expanded cards use progressive disclosure: summary (dose/water/grind/time/flavors/notes preview + Edit/Delete) visible on expand, full details behind "Show details" toggle. Edit navigates to BrewForm. Compare mode: select 2 brews for side-by-side diff.
...
- `src/components/StepEditor.jsx` — Shared step editor used in RecipeAssembly and BrewForm. Reads/writes unified step format (`{ id, name, waterTo, time, duration, note }`). Props: `cascadeTime` (auto-recalculate start times when duration changes), `plannedSteps` (show inline diff annotations comparing actuals vs plan).
...
- `src/components/TimeInput.jsx` — Reusable smart time input. Displays MM:SS, accepts "90" or "1:30" via `parseFlexTime()`, commits on blur. Conditional label wrapper. Used by StepEditor, BrewForm, RateThisBrew.
...
**Steps & results:** `steps` (actual steps — the single editable field, pre-filled from recipe at brew time, correctable post-brew), `recipeSnapshot.steps` (frozen planned steps — read-only reference for diff comparison), `stepResults` (per-step timing: `{ stepId: { tappedAt, skipped, variance } }`). Legacy `recipeSteps` field has been migrated away — `migrateDropRecipeSteps()` moves data to `recipeSnapshot.steps`.
...
**Migrations:** Run synchronously in App.jsx lazy initializer. Pattern: idempotent check → in-place mutation → batch write → return brews. Current chain: `migrateGrindSettings()` → `seedDefaultPourTemplates()` → `migrateBloomToSteps()` → `migrateToSchemaV2()` → `migrateExtractRecipes()` → `migrateDropRecipeSteps()`. V2 migration creates backup in `brewlog_brews_backup_v1` (does not overwrite existing backup). On corrupt data, attempts restore from backup. Recipe migration extracts unique recipe entities from existing brews grouped by beanId. `migrateDropRecipeSteps` removes the redundant `recipeSteps` field, synthesizing `recipeSnapshot.steps` from it where missing.
...
**Nullish coalescing for numeric state:** Use `??` not `||` when initializing or serializing form state that can legitimately be `0` (time, counts, measurements). `||` treats zero as falsy and discards it. See `docs/solutions/logic-errors/nullish-coalescing-required-for-numeric-form-state.md`.
